...
A 는 2 개의 부분을 서로 다른 부분으로 판단했고 그 중 정답은 2개 이므로 **Precision** 은 2/2 = 1.0 으로 100% 입니다. 

B 는 5 개의 부분을 서로 다른 부분으로 판단했고 그 중 정답은 3개 이므로 **Precision** 은 3/5 = 0.75 로 75% 입니다.
...
A 는 2 개의 개의 부분을 서로 다른 부분으로 판단했고 그 중 정답은 2개 이므로 **Recall** 은 2/3 = 0.67 로 67% 입니다. 

B 는 5 개의개의 부분을 서로 다른 부분으로 판단했고 그 중 정답은 3개 이므로 **Recall** 은 3/3 = 1.00 으로 100% 입니다.
...
**Recall** 은 문제를 풀었을때 **오답이 존재하든 말든 정답을 많이 맞출수록** 높은 점수를 주는 척도입니다.
