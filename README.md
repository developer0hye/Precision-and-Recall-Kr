...
**Precision** 은 문제를 풀었을때 **오답(정답이 아닌데 정답으로 보는 경우, 정답을 못맞춘 경우가 아님!)** 이 존재하면 페널티를 주는 

척도이며 또한 정답을 모두 못 맞췄더라도 **오답이 적다면** 높은 점수를 주는 척도입니다.

따라서 A 방법은 정답을 모두 못 맞췄지만 구해낸 답 중에서 오답이 없었고 B 방법은 정답을 맞추었지만 구해낸 답이 오답을 포함하고 

있으므로 **Precision** 관점에서는 A 방법이 더 좋은 방법입니다. 
