...
따라서 A 방법은 정답을 모두 못 맞췄지만 구해낸 답 중에서 오답이 없었고 B 방법은 정답을 맞추었지만 구해낸 답이 오답을 포함하고 있으므로 

**Precision** 관점에서는 A 방법이 더 좋은 방법입니다. 

**Recall** 은 문제를 풀었을때 오답이 존재하든 말든 정답을 많이 맞추는 경우 높은 점수를 주는 척도입니다.

따라서 B 방법은 3개의 답을 모두 맞추었고 A 방법은 2개의 답만 맞추었으므로

**Recall** 관점에서는 B 방법이 더 좋은 방법입니다.
